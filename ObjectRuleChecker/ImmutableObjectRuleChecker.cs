...
{
    public class ImmutableObjectRuleChecker<T> : IObjectRuleChecker<T> where T : class
    {
        private readonly List<ValidationRule<T>> validationRules;

        public ImmutableObjectRuleChecker(IEnumerable<ValidationRule<T>> rules)
        {
        ...
        /// <summary>
        /// Creates a new instance of the checker. Adds the new rule to the existing ones.
        /// </summary>
        ...
        public IObjectRuleChecker<T> IsValidWhen(Expression<Func<T, bool>> newRule)
        {
            ...
            var rules = validationRules.Concat(new List<ValidationRule<T>> {new ValidationRule<T>(newRule)});
            return new ImmutableObjectRuleChecker<T>(rules);
        }
